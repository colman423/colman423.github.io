...
(function (pxt) {
    var callbacks;
    var ghSetup = false;
    var CachedGithubDb = /** @class */ (function () {
        function CachedGithubDb(db) {
            this.db = db;
        }
        CachedGithubDb.prototype.loadAsync = function (repopath, tag, suffix, loader) {
            // only cache releases
            if (!/^v\d+\.\d+\.\d+$/.test(tag))
                return loader(repopath, tag);
            var key = "gh-" + suffix + "-" + repopath + "#" + tag;
            return callbacks.cacheGet(key)
                .then(function (json) {
                if (json) {
                    var p = pxt.Util.jsonTryParse(json);
                    if (p) {
        ...
        CachedGithubDb.prototype.loadConfigAsync = function (repopath, tag) {
            var _this = this;
            return this.loadAsync(repopath, tag, "pxt", function (r, t) { return _this.db.loadConfigAsync(r, t); });
        };
        CachedGithubDb.prototype.loadPackageAsync = function (repopath, tag) {
            var _this = this;
            return this.loadAsync(repopath, tag, "pkg", function (r, t) { return _this.db.loadPackageAsync(r, t); });
        };
        return CachedGithubDb;
    }());
    function pkgOverrideAsync(pkg) {
        var _a;
        var f = (_a = callbacks) === null || _a === void 0 ? void 0 : _a.pkgOverrideAsync;
        var v = f ? f(pkg.id) : Promise.resolve(undefined);
        return v.then(function (r) { return r ? r : pkg.commonDownloadAsync(); });
    }
    var SimpleHost = /** @class */ (function () {
        ...
        SimpleHost.prototype.writeFile = function (module, filename, contents) {
            var pref = module.id == "this" ? "" : "pxt_modules/" + module.id + "/";
        ...
        SimpleHost.prototype.cacheStoreAsync = function (id, val) {
            ...
            if ((_a = callbacks) === null || _a === void 0 ? void 0 : _a.cacheSet)
        ...
        SimpleHost.prototype.cacheGetAsync = function (id) {
            ...
            if ((_a = callbacks) === null || _a === void 0 ? void 0 : _a.cacheGet)
        ...
        SimpleHost.prototype.downloadPackageAsync = function (pkg) {
            ...
            if (ghSetup)
                return pkgOverrideAsync(pkg)
                    .then(function (resp) {
                    if (resp) {
                        pxt.U.iterMap(resp, function (fn, cont) {
    ...
    (function (pxtc) {
        ...
        var AssemblerSnippets = /** @class */ (function () {
            ...
            AssemblerSnippets.prototype.hex_literal = function (lbl, data) {
                ...
                var align = /f{16}/i.test(data) ? 8 : 4;
        ...
        var ProctoAssembler = /** @class */ (function () {
            ...
            ProctoAssembler.prototype.writeFailBranch = function () {
                this.write(".fail:");
            ...
            ProctoAssembler.prototype.emitArrayMethod = function (op, isBuffer) {
                ...
                var ldrSize = pxtc.isStackMachine() || pxtc.target.runtimeIsARM || isBuffer ? "ldr" : "ldrh";
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        function vmEmit(bin, opts) {
            ...
            bin.usedClassInfos.forEach(function (info) {
                section(vtName(info), SectionType.VTable, function () { return vtableToVM(info, opts, bin); });
            });
            pxtc.U.values(additionalClassInfos).forEach(function (info) {
                info.itable = [];
                info.classNo = 0xfff0;
                section(vtName(info), SectionType.VTable, function () { return vtableToVM(info, opts, bin); });
            });
        ...
        function irToVM(ctx, bin, proc) {
            ...
            function emitProcCall(topExpr) {
                ...
                if (calledProc && calledProc.inlineBody) {
                    var inlined = calledProc.inlineSelf(topExpr.args);
                    if (pxt.options.debug) {
    ...
    (function (pxtc) {
        ...
        (function (decompiler) {
            ...
            function decompileToBlocks(blocksInfo, file, options, renameMap) {
                ...
                var apis = blocksInfo.apis.byQName;
                Object.keys(apis).forEach(function (qName) {
                    var api = apis[qName];
                ...
                function attrs(callInfo) {
                    var blockInfo = blocksInfo.apis.byQName[callInfo.decompilerBlockAlias || callInfo.qName];
                    if (blockInfo) {
                ...
                function getCallStatement(node, asExpression) {
                    ...
                    var api = env.blocks.apis.byQName[info.decompilerBlockAlias || info.qName];
                    var comp = pxt.blocks.compileInfo(api);
                ...
                function checkCall(n, env, asExpression, topLevel) {
                    ...
                    if (!asExpression) {
                        if (info.isExpression && !userFunction) {
                            var alias = env.aliasBlocks[info.qName];
                            if (alias) {
        ...
        (function (ir) {
            ...
            var Procedure = /** @class */ (function (_super) {
                ...
                Procedure.prototype.inlineSelf = function (args) {
                    var _a = flattenArgs(args, false, true), precomp = _a.precomp, flattened = _a.flattened;
            ...
            function flattenArgs(args, reorder, keepcomplex) {
                if (reorder === void 0) { reorder = false; }
                if (keepcomplex === void 0) { keepcomplex = false; }
    ...
    (function (pxtc) {
        ...
        function compileBinary(program, opts, res, entryPoint) {
            ...
            if (opts.target.isNative) {
                ...
                var extinfo = opts.extinfo;
                var optstarget = opts.target;
                // if current (main) extinfo is disabled use another one
                if (extinfo && extinfo.disabledDeps) {
                    var enabled = (opts.otherMultiVariants || []).find(function (e) { return e.extinfo && !e.extinfo.disabledDeps; });
                    if (enabled) {
            ...
            if (!opts.forceEmit || res.diagnostics.length == 0) {
                var files = program.getSourceFiles().slice();
                var main = files.find(function (sf) { return sf.fileName === "main.ts"; });
                if (main) {
                    files = files.filter(function (sf) { return sf.fileName !== "main.ts"; });
                    files.push(main);
                }
                // run post-processing code last, if present
                var postProcessing = files.find(function (sf) { return sf.fileName === pxt.TUTORIAL_CODE_STOP; });
                if (postProcessing) {
                    files = files.filter(function (sf) { return sf.fileName !== pxt.TUTORIAL_CODE_STOP; });
            ...
            function emitCallCore(node, funcExpr, callArgs, sig, decl, recv) {
                ...
                function emitPlain() {
                    var r = mkProcCall(decl, node, args.map(function (x) { return emitExpr(x); }));
                    var pp = r.data;
                ...
                if (hasRecv) {
                    ...
                    if (attrs.shim && !hasShimDummy(decl)) {
                        ...
                        for (var _i = 0, syms_1 = syms; _i < syms_1.length; _i++) {
                            var sym = syms_1[_i];
                            if (sym.name == "helpers") {
                                for (var _b = 0, _c = sym.declarations || [sym.valueDeclaration]; _b < _c.length; _b++) {
                                    var d = _c[_b];
                                    if (d.kind == pxtc.SK.ModuleDeclaration) {
                                        for (var _d = 0, _e = d.body.statements; _d < _e.length; _d++) {
                                            var stmt = _e[_d];
            ...
            */
            ...
            function emitTaggedTemplateHelper(node, attrs) {
                ...
                var syms = checker.getSymbolsInScope(node, ts.SymbolFlags.Module);
                var helperStmt;
                for (var _i = 0, syms_2 = syms; _i < syms_2.length; _i++) {
                    var sym = syms_2[_i];
                    if (sym.name == "helpers") {
                        for (var _b = 0, _c = sym.declarations || [sym.valueDeclaration]; _b < _c.length; _b++) {
                            var d = _c[_b];
                ...
                var decl = helperStmt;
                markFunctionUsed(decl);
                var r = mkProcCall(decl, node, [emitStringLiteral(node.text)]);
                return r;
            }
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        function runConversionsAndStoreResults(opts, res) {
            ...
            // run post-processing code last, if present
            var postIdx = opts.sourceFiles.indexOf(pxt.TUTORIAL_CODE_STOP);
            if (postIdx >= 0) {
        ...
        function getTSProgram(opts, old) {
            ...
            // run post-processing code last, if present
            var post_idx = tsFiles.indexOf(pxt.TUTORIAL_CODE_STOP);
            if (post_idx >= 0) {
    ...
    :00000001FF
        ...
        */
        ...
        (function (hexfile) {
            ...
            function setupFor(opts, extInfo) {
                ...
                if (hexlines.length <= 2) {
                    var bytes = pxtc.U.fromHex(hexlines[0]);
                    if (bytes[2] <= 0x02 && bytes[3] == 0x60) {
                        var off = 0x60000000;
                        var page = 0x1000;
                        var endpadded = (bytes.length + page - 1) & ~(page - 1);
        ...
        function assembleAndPatch(src, bin, opts, cres) {
            var dummy = opts.extinfo.disabledDeps;
            if (dummy) {
                src =
                    hexfile.hexPrelude() + "\n" +
...
(function (ts_2) {
    ...
    (function (pxtc) {
        ...
        (function (service) {
            ...
            var COMPLETION_KEYWORD_WEIGHT = 0;
            var COMPLETION_DEFAULT_WEIGHT = 1;
            var COMPLETION_IN_SCOPE_VAR_WEIGHT = 5;
            var COMPLETION_MATCHING_PARAM_TYPE_WEIGHT = 10;
            function getCallSymbol(callExp) {
                var _a, _b, _c;
                var qName = (_c = (_b = (_a = callExp) === null || _a === void 0 ? void 0 : _a.pxt) === null || _b === void 0 ? void 0 : _b.callInfo) === null || _c === void 0 ? void 0 : _c.qName;
                var api = service.lastApiInfo.apis.byQName[qName];
                return api;
            }
            ...
            function getParameter(callSym, paramIdx, blocksInfo) {
                ...
                if (callSym.attributes._def) {
                    ...
                    if (shadowId) {
                        ...
                        var shadowApi = service.lastApiInfo.apis.byQName[shadowBlk.qName];
            ...
            function getApisForTsType(pxtType, location, tc, symbols, isEnum) {
                ...
                symbols.forEach(function (i) {
                    ...
                    if (isEnum) {
                        if (i.symbol.kind == 7 /* EnumMember */) {
                            ...
                            var enumParts = (_b = (_a = i.symbol.attributes) === null || _a === void 0 ? void 0 : _a.enumIdentity) === null || _b === void 0 ? void 0 : _b.split(".");
                            if (((_c = enumParts) === null || _c === void 0 ? void 0 : _c.length) > 1)
                                retType = enumParts[0];
                        }
                    }
                    apisByRetType[retType] = __spreadArrays((apisByRetType[retType] || []), [i]);
                });
                ...
                for (var _i = 0, retApis_1 = retApis; _i < retApis_1.length; _i++) {
                    ...
                    if (asTsEnum) {
                        ...
                        var mems = pxtc.getEnumMembers(tc, enumType);
                        var enumValQNames = mems.map(function (e) { return pxtc.enumMemberToQName(tc, e); });
                        var symbols_1 = enumValQNames.map(function (n) { return service.lastApiInfo.apis.byQName[n]; });
                        enumVals = __spreadArrays(enumVals, symbols_1);
                    }
                }
                return __spreadArrays(retApis, completionSymbols(enumVals, COMPLETION_DEFAULT_WEIGHT));
            }
            ...
            function getNodeAndSymbolAtLocation(program, filename, position, apiInfo) {
                ...
                var checker = program.getTypeChecker();
                var node = pxtc.findInnerMostNodeAtPosition(source, position);
                if (node) {
            ...
            function tsTypeToPxtTypeString(t, tc) {
                ...
                var tcString = tc.typeToString(t);
                var primativeToQname = {
                    "number": "Number",
                    "string": "String",
                    "boolean": "Boolean"
                };
                var pxtString = (_a = primativeToQname[tcString], (_a !== null && _a !== void 0 ? _a : tcString));
                return pxtString;
            }
            ...
            function getTsSymbolFromPxtSymbol(pxtSym, location, meaning) {
                var checker = service.service && service.service.getProgram().getTypeChecker();
                if (!checker)
                    return null;
            ...
            function getDefaultEnumValue(t, python) {
                ...
                var checker = service.service && service.service.getProgram().getTypeChecker();
                var members = pxtc.getEnumMembers(checker, t);
                for (var _i = 0, members_1 = members; _i < members_1.length; _i++) {
                    var member = members_1[_i];
                    if (member.name.kind === pxtc.SK.Identifier) {
                        var fullName = pxtc.enumMemberToQName(checker, member);
                        var pxtSym = service.lastApiInfo.apis.byQName[fullName];
                        if (pxtSym) {
            ...
            function getCompletions(v) {
                ...
                var lineText = src.substring(lastNl + 1, position);
                ...
                var isMemberCompletion = dotIdx !== -1;
                r.isMemberCompletion = isMemberCompletion;
                var partialWord = isMemberCompletion ? src.slice(dotIdx + 1, wordEndPos) : src.slice(wordStartPos, wordEndPos);
                var MAX_SYMBOLS_BEFORE_FILTER = 50;
                var MAX_SYMBOLS = 100;
                if (isMemberCompletion)
                    complPosition = dotIdx;
                var entries = {};
                var opts = service.cloneCompileOpts(service.host.opts);
                ...
                if (isPython) {
                    ...
                    if (res_5.syntaxInfo && res_5.syntaxInfo.symbols) {
                        resultSymbols = completionSymbols(res_5.syntaxInfo.symbols, COMPLETION_DEFAULT_WEIGHT);
                    }
                    if (res_5.globalNames)
                        service.lastGlobalNames = res_5.globalNames;
                    if (!resultSymbols.length && res_5.globalNames) {
                        resultSymbols = completionSymbols(pxt.U.values(res_5.globalNames), COMPLETION_DEFAULT_WEIGHT);
                    }
                    // update our language host
                    Object.keys(res_5.outfiles)
                        .forEach(function (k) {
                        if (k === tsFilename) {
                            service.host.setFile(k, res_5.outfiles[k]);
                        }
                    });
                    ...
                    if (!isMemberCompletion && resultSymbols.length > MAX_SYMBOLS_BEFORE_FILTER) {
                        resultSymbols = resultSymbols
                            .filter(function (s) { return (isPython ? s.symbol.pyQName : s.symbol.qName).toLowerCase().indexOf(partialWord.toLowerCase()) >= 0; });
                    }
                    opts.ast = true;
                    var ts2asm = pxtc.compile(opts, service.service);
                }
                else {
                    tsPos = position;
                    opts.ast = true;
                    service.host.setOpts(opts);
                    var res = service.runConversionsAndCompileUsingService();
                }
                var prog = service.service.getProgram();
                var tsAst = prog.getSourceFile(tsFilename);
                var tc = prog.getTypeChecker();
                var tsNode = pxtc.findInnerMostNodeAtPosition(tsAst, tsPos);
                var commentMap = pxtc.decompiler.buildCommentMap(tsAst);
                // abort if we're in a comment
                var inComment = commentMap.some(function (range) { return range.start <= position && position <= range.end; });
                if (inComment) {
                    return r;
                }
                // abort if we're in a string literal
                if (tsNode) {
                    var stringLiteralKinds = [pxtc.SK.StringLiteral, pxtc.SK.FirstTemplateToken, pxtc.SK.NoSubstitutionTemplateLiteral];
                    var inLiteral = stringLiteralKinds.some(function (k) { return tsNode.kind === k; });
                    if (inLiteral) {
                        return r;
                    }
                }
                ...
                var didFindMemberCompletions = false;
                if (isMemberCompletion) {
                    var propertyAccessTarget = pxtc.findInnerMostNodeAtPosition(tsAst, isPython ? tsPos : dotIdx - 1);
                    if (propertyAccessTarget) {
                        var type = void 0;
                        var symbol = tc.getSymbolAtLocation(propertyAccessTarget);
                        if (((_b = (_a = symbol) === null || _a === void 0 ? void 0 : _a.members) === null || _b === void 0 ? void 0 : _b.size) > 0) {
                        ...
                        if (type) {
                            var qname_1 = type.symbol ? tc.getFullyQualifiedName(type.symbol) : tsTypeToPxtTypeString(type, tc);
                            if (qname_1) {
                                var props = type.getApparentProperties()
                                    .map(function (prop) { return qname_1 + "." + prop.getName(); })
                                    .map(function (propQname) { return service.lastApiInfo.apis.byQName[propQname]; })
                                    .filter(function (prop) { return !!prop; })
                                    .map(function (prop) { return completionSymbol(prop, COMPLETION_DEFAULT_WEIGHT); });
                                resultSymbols = props;
                ...
                var allSymbols = pxt.U.values(service.lastApiInfo.apis.byQName);
                if (resultSymbols.length === 0) {
                    ...
                    var wordMatching = allSymbols.filter(function (s) { return (isPython ? s.pyQName : s.qName).toLowerCase().indexOf(partialWord.toLowerCase()) >= 0; });
                ...
                if (!isPython && !didFindMemberCompletions) {
                    ...
                    var inScopeTsSyms = tc.getSymbolsInScope(tsNode, symSearch);
                    // filter these to just what's at the cursor, otherwise we get things
                    //  like JS Array methods we don't support
                    var matchStr_1 = tsNode.getText();
                    if (matchStr_1 !== "_") // if have a real identifier ("_" is a placeholder we added), filter to prefix matches
                    ...
                    var inScopePxtSyms = inScopeTsSyms
                        .map(function (t) {
                        var pxtSym = getPxtSymbolFromTsSymbol(t, service.lastApiInfo.apis, tc);
                        if (!pxtSym) {
                            var tsType = tc.getTypeOfSymbolAtLocation(t, tsNode);
                            pxtSym = makePxtSymbolFromTsSymbol(t, tsType);
                        }
                        return pxtSym;
                    })
                        .filter(function (s) { return !!s; })
                        .map(function (s) { return completionSymbol(s, COMPLETION_DEFAULT_WEIGHT); });
                    // in scope locals should be weighter higher
                    inScopePxtSyms.forEach(function (s) { return s.weight += COMPLETION_IN_SCOPE_VAR_WEIGHT; });
                    resultSymbols = __spreadArrays(resultSymbols, inScopePxtSyms);
                }
                // special handling for call expressions
                var call = pxtc.getParentCallExpression(tsNode);
                if (call) {
                    // which argument are we ?
                    var paramIdx = pxtc.findCurrentCallArgIdx(call, tsNode, tsPos);
                    // if we're not one of the arguments, are we at the
                    // determine parameter idx
                    if (paramIdx >= 0) {
                        var blocksInfo_1 = service.blocksInfoOp(service.lastApiInfo.apis, runtime.bannedCategories);
                        var callSym = getCallSymbol(call);
                        if (callSym) {
                            if (paramIdx >= callSym.parameters.length)
                                paramIdx = callSym.parameters.length - 1;
                            var param = getParameter(callSym, paramIdx, blocksInfo_1); // shakao get param type
                            if (param) {
                                // weight the results higher if they return the correct type for the parameter
                                var matchingApis = getApisForTsType(param.type, call, tc, resultSymbols, param.isEnum);
                                matchingApis.forEach(function (match) { return match.weight = COMPLETION_MATCHING_PARAM_TYPE_WEIGHT; });
                            }
                        }
                    }
                }
                ...
                if (!isMemberCompletion) {
                    ...
                    var keywords = void 0;
                    if (isPython) {
                        var keywordsMap = pxt.py.keywords;
                        keywords = Object.keys(keywordsMap);
                    }
                    else {
                        keywords = __spreadArrays(ts.pxtc.reservedWords, ts.pxtc.keywordTypes);
                    }
                    var keywordSymbols = keywords
                        .filter(function (k) { return k.indexOf(partialWord) >= 0; })
                        .map(makePxtSymbolFromKeyword)
                        .map(function (s) { return completionSymbol(s, COMPLETION_KEYWORD_WEIGHT); });
                    resultSymbols = __spreadArrays(resultSymbols, keywordSymbols);
                }
                ...
                var _e = v.runtime, bannedCategories = _e.bannedCategories, screenSize = _e.screenSize;
                var blocksInfo = service.blocksInfoOp(service.lastApiInfo.apis, bannedCategories);
                var context = {
                    takenNames: takenNames,
                    blocksInfo: blocksInfo,
                    screenSize: screenSize,
                    apis: service.lastApiInfo.apis,
                    checker: (_d = (_c = service.service) === null || _c === void 0 ? void 0 : _c.getProgram()) === null || _d === void 0 ? void 0 : _d.getTypeChecker()
                };
                resultSymbols.forEach(function (sym) { return patchSymbolWithSnippet(sym.symbol, isPython, context); });
            ...
            function patchSymbolWithSnippet(si, isPython, context) {
                var n = service.lastApiInfo.decls[si.qName];
                if (ts_2.isFunctionLike(n)) {
                    ...
                    if (si.snippetAddsDefinitions
                        ...
                        || (!isPython && !si.snippet)) {
                        var snippetNode = service.getSnippet(context, si, n, isPython);
                        var snippet = service.snippetStringify(snippetNode);
                        var snippetWithMarkers = service.snippetStringify(snippetNode, true);
                        var addsDefinitions = service.snippetAddsDefinitions(snippetNode);
                        if (isPython) {
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        function emitPyTypeFromTypeNode(s) {
            ...
            switch (s.kind) {
                ...
                case ts.SyntaxKind.ArrayType: {
                    var t = s;
                    var elType = emitPyTypeFromTypeNode(t.elementType);
                    return "List[" + elType + "]";
                }
                case ts.SyntaxKind.TypeReference: {
                    var t = s;
                    var nm = t.typeName && t.typeName.getText ? t.typeName.getText() : "";
                    return nm;
                }
        ...
        function emitFuncPyType(s) {
            var returnType = emitPyTypeFromTypeNode(s.type);
            ...
            if (kind != 0 /* None */) {
                ...
                var r = {
                    ...
                    parameters: !hasParams ? null : pxtc.Util.toArray(decl.parameters).map(function (p, i) {
                        ...
                        var pyTypeString = (p.type && emitPyTypeFromTypeNode(p.type))
                            || (paramType && emitPyTypeFromTsType(paramType))
                            || "unknown";
                        var initializer = p.initializer ? p.initializer.getText() :
        ...
        function internalGetApiInfo(program, jres, legacyOnly) {
            ...
            var collectDecls = function (stmt) {
                ...
                if (pxtc.isExported(stmt)) {
                    ...
                    if (si_1) {
                        var existing_1 = pxtc.U.lookup(res.byQName, qName);
                        if (existing_1) {
                            // we can have a function and an interface of the same name
                            if (existing_1.kind == 9 /* Interface */ && si_1.kind != 9 /* Interface */) {
                                // save existing entry
                                res.byQName[qName + "@type"] = existing_1;
                            }
                            else if (existing_1.kind != 9 /* Interface */ && si_1.kind == 9 /* Interface */) {
                                res.byQName[qName + "@type"] = si_1;
                                si_1 = existing_1;
                            }
                            else {
                                var foundSrc = (_a = existing_1.attributes._source) === null || _a === void 0 ? void 0 : _a.trim();
                                var newSrc = (_b = si_1.attributes._source) === null || _b === void 0 ? void 0 : _b.trim();
                                var source = foundSrc + "\n" + newSrc;
                                // Avoid duplicating source if possible
                                if (!!foundSrc && ((_c = newSrc) === null || _c === void 0 ? void 0 : _c.indexOf(foundSrc)) >= 0) {
                                    source = newSrc;
                                }
                                else if (!!newSrc && ((_d = foundSrc) === null || _d === void 0 ? void 0 : _d.indexOf(newSrc)) >= 0) {
                                    source = foundSrc;
                                }
                                si_1.attributes = pxtc.parseCommentString(source);
                                ...
                                if (existing_1.kind === 5 /* Module */) {
                                    // Reference the existing array of packages where this namespace has been defined 
                                    si_1.pkgs = existing_1.pkgs || [];
                                    if (existing_1.pkg !== si_1.pkg) {
                                        if (!si_1.pkgs.find(function (element) { return element === existing_1.pkg; })) {
                                            si_1.pkgs.push(existing_1.pkg);
                                        }
                                    }
                                }
                                if (existing_1.extendsTypes) {
                                    si_1.extendsTypes = si_1.extendsTypes || [];
                                    existing_1.extendsTypes.forEach(function (t) {
    ...
    (function (pxtc) {
        ...
        (function (service_1) {
            ...
            function fileDiags(fn) {
                if (!/\.ts$/.test(fn))
                    return [];
                var d = service_1.service.getSyntacticDiagnostics(fn);
                if (!d || !d.length)
            ...
            var operations = {
                reset: function () {
                    service_1.service.cleanupSemanticCache();
                ...
                syntaxInfo: function (v) {
                    ...
                    var opts = cloneCompileOpts(service_1.host.opts);
                    ...
                    var isSymbolReq = opts.syntaxInfo.type === "symbol";
                    var isSignatureReq = opts.syntaxInfo.type === "signature";
                    if (isPython) {
                        var res = pxtc.transpile.pyToTs(opts);
                        if (res.globalNames)
                        ...
                        var prog = service_1.service.getProgram();
                        var tsAst = prog.getSourceFile(v.fileName);
                        var tc = prog.getTypeChecker();
                        if (isSymbolReq || isSignatureReq) {
                            var tsNode = pxtc.findInnerMostNodeAtPosition(tsAst, v.position);
                            if (tsNode) {
                                if (isSymbolReq) {
                                    var symbol = tc.getSymbolAtLocation(tsNode);
                                    if (symbol) {
                                        var pxtSym = service_1.getPxtSymbolFromTsSymbol(symbol, opts.apisInfo, tc);
                                    ...
                                    var pxtCall = (_b = (_a = tsNode) === null || _a === void 0 ? void 0 : _a.pxt) === null || _b === void 0 ? void 0 : _b.callInfo;
                                    if (pxtCall) {
                                        var pxtSym = opts.apisInfo.byQName[pxtCall.qName];
                                        opts.syntaxInfo.symbols = [pxtSym];
                                        opts.syntaxInfo.beginPos = tsNode.getStart();
                                        opts.syntaxInfo.endPos = tsNode.getEnd();
                                        var tsCall = pxtc.getParentCallExpression(tsNode);
                                        if (tsCall) {
                                            var argIdx = pxtc.findCurrentCallArgIdx(tsCall, tsNode, v.position);
                    ...
                    if (isSymbolReq && !((_c = opts.syntaxInfo.symbols) === null || _c === void 0 ? void 0 : _c.length)) {
                        var possibleKeyword = service_1.getWordAtPosition(v.fileContent, v.position);
                        if (possibleKeyword) {
                            ...
                            if (isPython && possibleKeyword.text === "range") {
                                var apiInfo = getLastApiInfo(opts).apis;
                                ...
                                var help = pxtc.getHelpForKeyword(possibleKeyword.text, isPython);
                                if (help) {
                    ...
                    if ((_d = opts.syntaxInfo.symbols) === null || _d === void 0 ? void 0 : _d.length) {
                        var apiInfo_1 = getLastApiInfo(opts).apis;
                        if (isPython) {
                            opts.syntaxInfo.symbols = opts.syntaxInfo.symbols.map(function (s) {
                                ...
                                return apiInfo_1.byQName[s.qName] || s;
                            });
                        }
                        if (isSymbolReq) {
                            opts.syntaxInfo.auxResult = opts.syntaxInfo.symbols.map(function (s) {
                                return service_1.displayStringForSymbol(s, isPython, apiInfo_1);
                            });
                        }
                    }
                ...
                snippet: function (v) {
                    ...
                    if (!service_1.lastApiInfo)
                        return undefined;
                    var fn = service_1.lastApiInfo.apis.byQName[o.qName];
                    var n = service_1.lastApiInfo.decls[o.qName];
                    ...
                    var _a = v.runtime, bannedCategories = _a.bannedCategories, screenSize = _a.screenSize;
                    var apis = service_1.lastApiInfo.apis;
                    var blocksInfo = blocksInfoOp(apis, bannedCategories);
                    var checker = service_1.service && service_1.service.getProgram().getTypeChecker();
                    var snippetContext = {
                        apis: apis,
                        blocksInfo: blocksInfo,
                        takenNames: takenNames,
                    ...
                    var snippetNode = service_1.getSnippet(snippetContext, fn, n, isPython);
                    var snippet = service_1.snippetStringify(snippetNode);
                    return snippet;
                },
                ...
                apiSearch: function (v) {
                    ...
                    if (!service_1.builtinItems) {
                        ...
                        var _loop_8 = function (id) {
                            var blockDef = service_1.blockDefinitions[id];
                            if (blockDef.operators) {
                                var _loop_9 = function (op) {
                                    var opValues = blockDef.operators[op];
                                    opValues.forEach(function (v) { return service_1.builtinItems.push({
                                        id: id,
                                        name: blockDef.name,
                        ...
                        for (var id in service_1.blockDefinitions) {
                            _loop_8(id);
                        }
                    }
                    var subset;
                    ...
                    if (!lastFuse || search.subset) {
                        ...
                        if (search.subset) {
                            ...
                            builtinSearchSet = service_1.builtinItems.filter(function (s) { return !!service_1.tbSubset[s.id]; });
                        }
                        if (service_1.tbSubset) {
                            subset = blockInfo.blocks.filter(function (s) { return !!service_1.tbSubset[s.attributes.blockId]; });
                        }
                        else {
            ...
            function runConversionsAndCompileUsingService() {
                ...
                var prevFS = pxtc.U.flatClone(service_1.host.opts.fileSystem);
                var res = pxtc.runConversionsAndStoreResults(service_1.host.opts);
                if (res && res.globalNames) {
                    service_1.lastGlobalNames = res.globalNames;
                }
                var newFS = service_1.host.opts.fileSystem;
                service_1.host.opts.fileSystem = prevFS;
                for (var _i = 0, _a = Object.keys(newFS); _i < _a.length; _i++) {
                    var k = _a[_i];
                    service_1.host.setFile(k, newFS[k]);
                } // update version numbers
                if (res.diagnostics.length == 0) {
                    service_1.host.opts.skipPxtModulesEmit = false;
                    service_1.host.opts.skipPxtModulesTSC = false;
                    var currKey = service_1.host.opts.target.isNative ? "native" : "js";
                    ...
                    var ts2asm = pxtc.compile(service_1.host.opts, service_1.service);
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        (function (service) {
            ...
            function snippetStringify(snippet, emitMonacoReplacementPoints) {
                if (emitMonacoReplacementPoints === void 0) { emitMonacoReplacementPoints = false; }
                var namesToReplacementNumbers = {};
                var nextNum = 1;
                ...
                function internalSnippetStringify(snippet) {
                    ...
                    if (isSnippetReplacePoint(snippet)) {
                        if (emitMonacoReplacementPoints) {
                            ...
                            var name_6 = snippetStringify(snippet.default, false);
                            var num = namesToReplacementNumbers[name_6];
                            if (!num || snippet.isLiteral) {
                                num = nextNum;
                                nextNum++;
                                namesToReplacementNumbers[name_6] = num;
                            }
                            if (name_6.indexOf(".") >= 0 && name_6.indexOf(" ") < 0) {
                        ...
                        return snippet
                            .map(function (s) { return internalSnippetStringify(s); })
                            .join("");
                    }
                    else {
                        return snippet;
            ...
            function snippetHasReplacementPoints(snippet) {
                if (isSnippetReplacePoint(snippet)) {
                    ...
                    return snippet
                        ...
                        .reduce(function (p, n) { return p || n; }, false);
                }
                else {
                    return false;
                }
            ...
            function getSnippet(context, fn, decl, python, recursionDepth) {
                ...
                var apis = context.apis, takenNames = context.takenNames, blocksInfo = context.blocksInfo, screenSize = context.screenSize, checker = context.checker;
                var PY_INDENT = pxt.py.INDENT;
                var fileType = python ? "python" : "typescript";
                var preStmt = [];
                    ...
                    if (element.attributes.defaultInstance) {
                        ...
                        if (nsInfo_1.attributes.fixedInstances) {
                            ...
                            if (instanceToUse) {
                                ...
                                namespaceToUse = nsInfo_1.namespace;
                            }
                            if (namespaceToUse) {
                                addNamespace = true;
                            }
                ...
                var preDefinedSnippet = attrs && (python ? attrs.pySnippet : attrs.snippet);
                var snippet;
                if (preDefinedSnippet) {
                    ...
                    snippet = [fnName];
                    if (((_a = args) === null || _a === void 0 ? void 0 : _a.length) || element.kind == 1 /* Method */ || element.kind == 3 /* Function */ || element.kind == 8 /* Class */) {
                        var argsWithCommas = args.reduce(function (p, n) { return __spreadArrays(p, [p.length ? ", " : "", n]); }, []);
                        snippet = snippet.concat(__spreadArrays(["("], argsWithCommas, [")"]));
                    }
                }
                var insertText = snippetPrefix ? __spreadArrays([snippetPrefix, "."], snippet) : snippet;
                insertText = addNamespace ? __spreadArrays([firstWord(namespaceToUse), "."], insertText) : insertText;
                if (attrs && attrs.blockSetVariable) {
                    if (python) {
                        var varName = getUniqueName(pxtc.snakify(attrs.blockSetVariable));
                        var varNode = {
                            default: varName,
                            isDefinition: true
                        };
                        insertText = __spreadArrays([varNode, " = "], insertText);
                    }
                    else {
                        var varName = getUniqueName(attrs.blockSetVariable);
                        var varNode = {
                            default: varName,
                            isDefinition: true
                        };
                        insertText = __spreadArrays(["let ", varNode, " = "], insertText);
                    }
                }
                ...
                function getParameterDefault(param) {
                    ...
                    if ((_b = (_a = attrs) === null || _a === void 0 ? void 0 : _a.paramDefl) === null || _b === void 0 ? void 0 : _b[name]) {
                        var deflKind = void 0;
                        if (typeNode.kind == pxtc.SK.AnyKeyword) {
                            var defaultName = attrs.paramDefl[name].toUpperCase();
                    ...
                    var shadowDefFromFieldEditor = getDefaultValueFromFieldEditor(name);
                    ...
                    if (shadowSymbol) {
                        var tsSymbol = service.getTsSymbolFromPxtSymbol(shadowSymbol, param, ts.SymbolFlags.Enum);
                        if (tsSymbol) {
                        ...
                        var shadowAttrs = shadowSymbol.attributes;
                        if (shadowAttrs.shim === "KIND_GET" && shadowAttrs.blockId) {
                            var kindNamespace_1 = shadowAttrs.kindNamespace || fn.namespace;
                            var defaultValueForKind = pxtc.Util.values(apis.byQName).find(function (api) { return api.namespace === kindNamespace_1 && api.attributes.isKind; });
                        ...
                        if (recursionDepth < 3 && service.lastApiInfo.decls[shadowSymbol.qName]) {
                            var snippet_1 = getSnippet(context, shadowSymbol, service.lastApiInfo.decls[shadowSymbol.qName], python, recursionDepth + 1);
                            if (snippet_1)
                                return snippet_1;
                        }
                    }
                    ...
                    var basicRes = service.getBasicKindDefault(typeNode.kind, python);
                ...
                function getDefaultValueOfType(type) {
                    ...
                    if (type.symbol && type.symbol.flags & ts.SymbolFlags.Enum) {
                        var defl = service.getDefaultEnumValue(type, python);
                        return defl;
                    }
                    var typeSymbol = service.getPxtSymbolFromTsSymbol(type.symbol, apis, checker);
                    ...
                    if (typeSymbol && typeSymbol.attributes.fixedInstances) {
                        var fixedSyms = getFixedInstancesOf(typeSymbol);
                        if (fixedSyms.length) {
                            var defl = fixedSyms[0];
                ...
                function snippetFromSpriteEditorParams(opts) {
                    ...
                    if ((_a = opts) === null || _a === void 0 ? void 0 : _a.sizes) {
                    ...
                    parsed.initColor = withDefault((_b = opts) === null || _b === void 0 ? void 0 : _b.initColor, parsed.initColor);
                    parsed.initWidth = withDefault((_c = opts) === null || _c === void 0 ? void 0 : _c.initWidth, parsed.initWidth);
                    parsed.initHeight = withDefault((_d = opts) === null || _d === void 0 ? void 0 : _d.initHeight, parsed.initHeight);
                ...
                function getDefaultValueFromFieldEditor(paramName) {
                    var _a, _b, _c;
                    var compileInfo = pxt.blocks.compileInfo(fn);
                    var blockParam = (_a = compileInfo.parameters) === null || _a === void 0 ? void 0 : _a.find(function (p) { return p.actualName === paramName; });
                    if (!((_b = blockParam) === null || _b === void 0 ? void 0 : _b.shadowBlockId))
                        return null;
                    var sym = blocksById[blockParam.shadowBlockId];
                    if (!sym)
                        return null;
                    var fieldEditor = (_c = sym.attributes) === null || _c === void 0 ? void 0 : _c.paramFieldEditor;
                    if (!fieldEditor)
                        return null;
                    var fieldEditorName = fieldEditor[paramName];
                    if (!fieldEditorName)
                        return null;
                    var fieldEditorOptions = sym.attributes.paramFieldEditorOptions || {};
                ...
                function createDefaultFunction(functionSignature, isArgument) {
                    ...
                    if (python) {
                        ...
                        var enumParams = includedParameters.filter(function (p) {
                            ...
                            var snippet = snippetStringify(getParameterDefault(p));
                            var str = snippet.toLowerCase();
                        ...
                        n = pxtc.snakify(n);
                        n = getUniqueName(n);
                        preStmt = __spreadArrays(preStmt, [
                            preStmt.length ? "\n" : "",
...
(function (ts) {
    ...
    (function (pxtc) {
        ...
        function findCurrentCallArgIdx(call, tsNode, tsPos) {
            ...
            var paramIdx = call.arguments
                .map(function (a) { return a === tsNode; })
                .indexOf(true);
            if (paramIdx >= 0)
                return paramIdx;
            // is our cursor within the argument range?
            var inRange = call.arguments.pos <= tsPos && tsPos < call.end;
            if (!inRange)
                return -1;
            // no arguments?
        ...
        function getParentNamespace(n) {
            return traverseUp(n, function (n) { return ts.isModuleDeclaration(n) ? TraverseCheck.Found : TraverseCheck.Continue; });
        }
        pxtc.getParentNamespace = getParentNamespace;
        function getCurrentNamespaces(n) {
            if (!n)
                return [];
            var parent = getParentNamespace(n);
            if (!parent)
                return [];
            var ns = parent.name.getText();
            return __spreadArrays(getCurrentNamespaces(parent.parent), [ns]);
        }
