    ...
    (function (blocks_1) {
        ...
        function compileWorkspace(e, w, blockInfo) {
            try {
                ...
                if (pxt.react.getTilemapProject) {
                    pxt.react.getTilemapProject().removeInactiveBlockAssets(allBlocks.map(function (b) { return b.id; }));
                }
        ...
        function escapeVariables(current, e) {
            ...
            function escapeVarName(originalName) {
                if (!originalName)
                    return '_';
                var n = ts.pxtc.escapeIdentifier(originalName);
        ...
        (function (layout) {
            ...
            function flowBlocks(comments, blocks, ratio, maxWidth) {
                ...
                blocks.forEach(function (block) {
                    var refs = blocks_4.getBlockData(block).commentRefs;
                    if (refs.length) {
                        var children = [];
...
(function (pxt) {
    ...
    (function (blocks) {
        ...
        function injectBlocks(blockInfo) {
            ...
            return blockInfo.blocks
                ...
                if (fn.attributes.blockBuiltin) {
                    ...
                    var builtin = builtinBlocks()[fn.attributes.blockId];
                    builtin.symbol = fn;
        ...
        function initFunctions() {
             ...
             * @package
             */
            var makeCreateCallOptionOriginal = Blockly.Functions.makeCreateCallOption;
            ...
            Blockly.Functions.makeCreateCallOption = function (block) {
                var option = makeCreateCallOptionOriginal(block);
    ...
    (function (blocks) {
        ...
        function initExpandableBlock(info, b, def, comp, toggle, addInputs) {
            ...
            function updateButtons() {
                ...
                var showPlus = visibleOptions !== totalOptions;
...
(function (pxtblockly) {
    var FieldBase = /** @class */ (function (_super) {
        ...
        FieldBase.prototype.dispose = function () {
            this.onDispose();
        };
        FieldBase.prototype.getValue = function () {
            return this.valueText;
        };
        FieldBase.prototype.doValueUpdate_ = function (newValue) {
            if (newValue === null)
                return;
            this.valueText = this.loaded ? this.onValueChanged(newValue) : newValue;
        };
        FieldBase.prototype.getDisplayText_ = function () {
            return this.valueText;
        };
        FieldBase.prototype.onLoadedIntoWorkspace = function () {
            this.loaded = true;
            this.valueText = this.onValueChanged(this.valueText);
        };
        FieldBase.prototype.isInitialized = function () {
            return !!this.fieldGroup_;
        };
        FieldBase.prototype.getBlockData = function () {
            return pxt.blocks.getBlockDataForField(this.sourceBlock_, this.name);
        };
        FieldBase.prototype.setBlockData = function (value) {
...
(function (pxtblockly) {
    ...
    var FieldAssetEditor = /** @class */ (function (_super) {
        __extends(FieldAssetEditor, _super);
        function FieldAssetEditor(text, params, validator) {
            var _this = _super.call(this, text, params, validator) || this;
            _this.pendingEdit = false;
            _this.isEmpty = false;
            _this.assetChangeListener = function () {
                if (_this.pendingEdit)
                    return;
                var id = _this.getBlockData();
                if (id) {
                    _this.asset = pxt.react.getTilemapProject().lookupAsset(_this.getAssetType(), id);
                }
                _this.redrawPreview();
            };
            _this.lightMode = params.lightMode;
            _this.params = _this.parseFieldOptions(params);
            _this.blocksInfo = params.blocksInfo;
            return _this;
        }
        FieldAssetEditor.prototype.onInit = function () {
            this.redrawPreview();
        };
        FieldAssetEditor.prototype.onValueChanged = function (newValue) {
            this.parseValueText(newValue);
            this.redrawPreview();
            return this.getValueText();
        };
        FieldAssetEditor.prototype.showEditor_ = function () {
            var _this = this;
            if (this.isGreyBlock)
                return;
            ...
            switch (this.asset.type) {
                ...
                case "tilemap" /* Tilemap */:
                    ...
                    var project = pxt.react.getTilemapProject();
                    var allTiles = project.getProjectTiles(this.asset.data.tileset.tileWidth, true);
                    this.asset.data.projectReferences = [];
                    var _loop_3 = function (tile) {
                        if (!this_2.asset.data.tileset.tiles.some(function (t) { return t.id === tile.id; })) {
                            this_2.asset.data.tileset.tiles.push(tile);
                        }
                        if (project.isAssetUsed(tile, null, [this_2.asset.id])) {
                            this_2.asset.data.projectReferences.push(tile.id);
                        }
                    };
                    var this_2 = this;
            ...
            var fv = pxt.react.getFieldEditorView(editorKind, this.asset, this.params);
            ...
            fv.onHide(function () {
                ...
                if (result) {
                    ...
                    if (pxt.assetEquals(_this.asset, result))
                        return;
                    _this.pendingEdit = true;
                    if ((_a = result.meta) === null || _a === void 0 ? void 0 : _a.displayName)
                        _this.disposeOfTemporaryAsset();
                    _this.asset = result;
                    var lastRevision = project.revision();
                    _this.onEditorClose(_this.asset);
                    _this.updateAssetListener();
                    _this.updateAssetMeta();
                    _this.redrawPreview();
        ...
        FieldAssetEditor.prototype.render_ = function () {
        ...
        FieldAssetEditor.prototype.getDisplayText_ = function () {
            // This is only used when isGreyBlock is true
            var text = pxt.Util.htmlUnescape(this.valueText);
            return text.substr(0, text.indexOf("(")) + "(...)";
            ;
        };
        FieldAssetEditor.prototype.updateEditable = function () {
        ...
        FieldAssetEditor.prototype.getValue = function () {
            if (this.isGreyBlock)
                return pxt.Util.htmlUnescape(this.valueText);
            return this.getValueText();
        };
        FieldAssetEditor.prototype.onDispose = function () {
            var _a;
        ...
        FieldAssetEditor.prototype.disposeOfTemporaryAsset = function () {
        ...
        FieldAssetEditor.prototype.clearTemporaryAssetData = function () {
            if (this.isTemporaryAsset()) {
                this.setBlockData(null);
            }
        };
        FieldAssetEditor.prototype.onEditorClose = function (newValue) {
            // Subclass
        };
        FieldAssetEditor.prototype.redrawPreview = function () {
            if (!this.fieldGroup_)
                return;
        ...
        FieldAssetEditor.prototype.parseValueText = function (newText) {
            var _this = this;
            newText = pxt.Util.htmlUnescape(newText);
            if (this.sourceBlock_ && !this.sourceBlock_.isInFlyout) {
                var project = pxt.react.getTilemapProject();
                var id = this.getBlockData();
                var existing = project.lookupAsset(this.getAssetType(), id);
                if (existing && !(newText && this.isEmpty)) {
                    ...
                    if (this.asset) {
                        if (this.sourceBlock_ && this.asset.meta.blockIDs) {
                            this.asset.meta.blockIDs = this.asset.meta.blockIDs.filter(function (id) { return id !== _this.sourceBlock_.id; });
        ...
        FieldAssetEditor.prototype.parseFieldOptions = function (opts) {
        ...
        FieldAssetEditor.prototype.updateAssetMeta = function () {
            var _this = this;
            if (!this.asset)
                return;
            if (!this.asset.meta) {
            ...
            if (this.sourceBlock_) {
                if (this.asset.meta.blockIDs.indexOf(this.sourceBlock_.id) === -1) {
                    var blockIDs = this.asset.meta.blockIDs;
                    if (blockIDs.length && this.isTemporaryAsset() && blockIDs.some(function (id) { return _this.sourceBlock_.workspace.getBlockById(id); })) {
...
(function (pxtblockly) {
    ...
    var FieldAnimationEditor = /** @class */ (function (_super) {
        ...
        function FieldAnimationEditor() {
            var _this = _super !== null && _super.apply(this, arguments) || this;
            _this.onMouseEnter = function () {
                if (_this.animateRef || !_this.asset)
                    return;
                var assetInterval = _this.getParentInterval() || _this.asset.interval;
                var interval = assetInterval > 50 ? assetInterval : 50;
                var index = 0;
        ...
        FieldAnimationEditor.prototype.getAssetType = function () {
            return "animation" /* Animation */;
        };
        FieldAnimationEditor.prototype.createNewAsset = function (text) {
            var project = pxt.react.getTilemapProject();
            if (text) {
                var existing = pxt.lookupProjectAssetByTSReference(text, project);
                if (existing)
                    return existing;
                var frames_1 = parseImageArrayString(text);
                if (frames_1 && frames_1.length) {
                    return project.createNewAnimationFromData(frames_1, this.getParentInterval());
                }
                var asset = project.lookupAssetByName("animation" /* Animation */, text.trim());
                if (asset)
                    return asset;
            }
            return project.createNewAnimation(this.params.initWidth, this.params.initHeight);
        };
        FieldAnimationEditor.prototype.onEditorClose = function (newValue) {
            this.setParentInterval(newValue.interval);
        };
        FieldAnimationEditor.prototype.getValueText = function () {
            if (!this.asset)
                return "[]";
            if (this.isTemporaryAsset()) {
                return "[" + this.asset.frames.map(function (frame) {
        ...
        FieldAnimationEditor.prototype.redrawPreview = function () {
            ...
            if (this.asset) {
                this.frames = this.asset.frames.map(function (frame) { return pxtblockly.bitmapToImageURI(pxt.sprite.Bitmap.fromData(frame), PREVIEW_WIDTH, _this.lightMode); });
        ...
        FieldAnimationEditor.prototype.parseFieldOptions = function (opts) {
            return parseFieldOptions(opts);
        };
        return FieldAnimationEditor;
    }(pxtblockly.FieldAssetEditor));
    ...
    var FieldGridPicker = /** @class */ (function (_super) {
        ...
        FieldGridPicker.prototype.populateRow = function (row, options, tableContainer) {
            ...
            var _loop_4 = function (i) {
                ...
                // Set colour
                var backgroundColour = this_3.backgroundColour_;
                if (value == this_3.getValue()) {
                    // This option is selected
                    menuItem.setAttribute('aria-selected', 'true');
                    pxt.BrowserUtils.addClass(menuItem, 'goog-option-selected');
                    backgroundColour = this_3.sourceBlock_.getColourTertiary();
                    // Save so we can scroll to it later
                    this_3.selectedItemDom = menuItem;
                    if (hasImages && !this_3.shouldShowTooltips()) {
                        this_3.updateSelectedBar_(content, value);
                    }
                }
                menuItem.style.backgroundColor = backgroundColour;
                menuItem.style.borderColor = this_3.borderColour_;
                if (hasImages) {
                    // An image, not text.
                    ...
                    if (!('IntersectionObserver' in window)) {
                        ...
                        this_3.observer.observe(buttonImg);
                    }
                ...
                if (this_3.shouldShowTooltips()) {
                    Blockly.bindEvent_(menuItem, 'click', this_3, this_3.buttonClickAndClose_);
                    // Setup hover tooltips
                    var xOffset_1 = (this_3.sourceBlock_.RTL ? -this_3.tooltipConfig_.xOffset : this_3.tooltipConfig_.xOffset);
                    var yOffset_1 = this_3.tooltipConfig_.yOffset;
                    Blockly.bindEvent_(menuItem, 'mousemove', this_3, function (e) {
                        if (hasImages) {
                    ...
                    Blockly.bindEvent_(menuItem, 'mouseout', this_3, function (e) {
                        if (hasImages) {
                            // Hide the tooltip
                    ...
                    if (hasImages) {
                        ...
                        this_3.selectedBar_.style.display = '';
                        // Show the selected item (in the selected bar)
                        Blockly.bindEvent_(menuItem, 'click', this_3, function (e) {
                        ...
                        Blockly.bindEvent_(menuItem, 'click', this_3, this_3.buttonClickAndClose_);
                        Blockly.bindEvent_(menuItem, 'mouseup', this_3, this_3.buttonClickAndClose_);
                    }
                }
                menuItem.appendChild(menuItemContent);
                rowContent.appendChild(menuItem);
                if (i == 0) {
                    this_3.firstItem_ = menuItem;
                }
            };
            var this_3 = this;
...
(function (pxtblockly) {
    var FieldPosition = /** @class */ (function (_super) {
        ...
        FieldPosition.prototype.showEditor_ = function (_opt_e) {
            ...
            _super.prototype.showEditor_.call(this, _opt_e);
...
(function (pxtblockly) {
    var FieldSpriteEditor = /** @class */ (function (_super) {
        ...
        function FieldSpriteEditor() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        FieldSpriteEditor.prototype.getAssetType = function () {
            return "image" /* Image */;
        };
        FieldSpriteEditor.prototype.createNewAsset = function (text) {
            var project = pxt.react.getTilemapProject();
            if (text) {
                var asset = pxt.lookupProjectAssetByTSReference(text, project);
                if (asset)
                    return asset;
            }
            ...
            var bmp = text ? pxt.sprite.imageLiteralToBitmap(text) : new pxt.sprite.Bitmap(this.params.initWidth, this.params.initHeight);
            if (!bmp) {
                this.isGreyBlock = true;
                this.valueText = text;
                return undefined;
            }
            var newAsset = project.createNewProjectImage(bmp.data());
            return newAsset;
        };
        FieldSpriteEditor.prototype.getValueText = function () {
        ...
        FieldSpriteEditor.prototype.parseFieldOptions = function (opts) {
            return parseFieldOptions(opts);
        };
...
(function (pxtblockly) {
    var FieldTilemap = /** @class */ (function (_super) {
        ...
        FieldTilemap.prototype.getAssetType = function () {
            return "tilemap" /* Tilemap */;
        };
        FieldTilemap.prototype.createNewAsset = function (newText) {
            ...
            var project = pxt.react.getTilemapProject();
            var existing = pxt.lookupProjectAssetByTSReference(newText, project);
            if (existing)
                return existing;
            var tilemap = pxt.sprite.decodeTilemap(newText, "typescript", project) || project.blankTilemap(this.params.tileWidth, this.params.initWidth, this.params.initHeight);
            var newAsset;
            // Ignore invalid bitmaps
            if (checkTilemap(tilemap)) {
                this.initText = newText;
                this.isGreyBlock = false;
                var name_11 = project.createNewTilemapFromData(tilemap)[0];
                newAsset = project.getTilemap(name_11);
            }
        ...
        FieldTilemap.prototype.onEditorClose = function (newValue) {
            var result = newValue.data;
            var project = pxt.react.getTilemapProject();
        ...
        FieldTilemap.prototype.getValueText = function () {
            if (this.isGreyBlock)
                return pxt.Util.htmlUnescape(this.valueText);
            if (this.asset) {
                return pxt.getTSReferenceForAsset(this.asset);
        ...
        FieldTilemap.prototype.parseFieldOptions = function (opts) {
            return parseFieldOptions(opts);
        };
        return FieldTilemap;
    }(pxtblockly.FieldAssetEditor));
...
(function (pxtblockly) {
    ...
    var FieldTileset = /** @class */ (function (_super) {
        ...
        function FieldTileset(text, options, validator) {
            ...
            _this.assetChangeListener = function () {
                _this.doValueUpdate_(_this.getValue());
                _this.forceRerender();
            };
        ...
        FieldTileset.prototype.getValue = function () {
            if (this.selectedOption_) {
                var tile = this.selectedOption_[2];
        ...
        FieldTileset.prototype.doValueUpdate_ = function (newValue) {
            ...
            if (newValue) {
                ...
                var match = pxt.parseAssetTSReference(newValue);
                if (match) {
                    newValue = match.name;
                }
        ...
        FieldTileset.prototype.getOptions = function (opt_useCache) {
        ...
        FieldTileset.prototype.dispose = function () {
            _super.prototype.dispose.call(this);
            pxt.react.getTilemapProject().removeChangeListener("tile" /* Tile */, this.assetChangeListener);
        };
        FieldTileset.prototype.updateAssetListener = function () {
            var project = pxt.react.getTilemapProject();
...
(function (pxtblockly) {
    ...
    function promptAndCreateEnum(ws, opts, message, cb) {
        Blockly.prompt(message, null, function (response) {
            if (response) {
                ...
                for (var i = 0; i < existing.length; i++) {
                    var _a = existing[i], name_12 = _a[0], value = _a[1];
                    if (name_12 === response) {
    ...
    function getVariableNameForMember(ws, enumName, memberName) {
        ...
        if (existing && existing.length) {
            for (var i = 0; i < existing.length; i++) {
                var name_13 = parseName(existing[i])[0];
                if (name_13 === memberName) {
        ...
        try {
            ...
            for (var _i = 0, allTiles_1 = allTiles; _i < allTiles_1.length; _i++) {
                ...
                if (tile.qualifiedName) {
                    customMapping_1[tile.projectId] = proj.resolveTile(tile.qualifiedName);
                }
                else if (tile.data) {
                    customMapping_1[tile.projectId] = proj.createNewTile(tile.data, "myTiles.tile" + tile.projectId);
                }
                deleteTilesetTileIfExists(ws, tile);
            }
            var tilemaps = getAllBlocksWithTilemaps(ws);
            var _loop_7 = function (tilemap) {
                ...
                var newData = new pxt.sprite.TilemapData(legacy.tilemap, {
                    ...
                    tiles: legacy.tileset.tiles.map(function (t, index) {
                        if (t.projectId != null) {
                            return customMapping_1[t.projectId];
                        }
                        if (!mapping[index]) {
    ...
    function getAllReferencedTiles(workspace, excludeBlockID) {
        ...
        for (var _i = 0, allMaps_1 = allMaps; _i < allMaps_1.length; _i++) {
            ...
            if (map.block.id === excludeBlockID)
                continue;
            for (var _b = 0, _c = ((_a = map.ref.getTileset()) === null || _a === void 0 ? void 0 : _a.tiles) || []; _b < _c.length; _b++) {
                var tile = _c[_b];
                all[tile.id] = project.lookupAsset("tile" /* Tile */, tile.id);
            }
        }
        var projectMaps = project.getAssets("tilemap" /* Tilemap */);
        for (var _d = 0, projectMaps_1 = projectMaps; _d < projectMaps_1.length; _d++) {
            var projectMap = projectMaps_1[_d];
            for (var _e = 0, _f = projectMap.data.tileset.tiles; _e < _f.length; _e++) {
                var tile = _f[_e];
                all[tile.id] = project.lookupAsset("tile" /* Tile */, tile.id);
            }
        }
        var allTiles = getAllBlocksWithTilesets(workspace);
        for (var _g = 0, allTiles_2 = allTiles; _g < allTiles_2.length; _g++) {
            var tilesetField = allTiles_2[_g];
            var value = tilesetField.ref.getValue();
            var match = /^\s*assets\s*\.\s*tile\s*`([^`]*)`\s*$/.exec(value);
            if (match) {
                var tile = project.lookupAssetByName("tile" /* Tile */, match[1]);
                if (tile && !all[tile.id]) {
                    all[tile.id] = tile;
                }
            }
        ...
        return Object.keys(all).map(function (key) { return all[key]; }).filter(function (t) { return !!t; });
    }
