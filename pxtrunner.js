...
(function (pxt) {
    ...
    (function (runner) {
        ...
        function fillWithWidget(options, $container, $js, $py, $svg, decompileResult, woptions) {
            ...
            if (woptions.showEdit && !theme.hideDocsEdit && decompileResult) { // edit button
                ...
                if (options.assetJSON) {
                    for (var _i = 0, _a = Object.keys(options.assetJSON); _i < _a.length; _i++) {
                        var key = _a[_i];
            ...
            if (woptions.run && !theme.hideDocsSimulator) {
                var $runBtn = snippetBtn(lf("Run"), "play icon").click(function () {
                    ...
                    if ($c.find('.sim')[0]) {
                        ...
                        var assets = options.assetJSON ? "data-assets=\"" + encodeURIComponent(JSON.stringify(options.assetJSON)) + "\"" : "";
        ...
        function renderApisAsync(options, replaceParent) {
            ...
            return decompileApiAsync(options)
                ...
                var symbols = pxt.Util.values(info.apis.byQName)
                    .filter(function (symbol) { return !symbol.attributes.hidden
                ...
                apisEl.each(function (i, e) {
                    ...
                    var csymbols = symbols.filter(function (symbol) { return !!namespaces[symbol.attributes.blockNamespace || symbol.namespace]; });
                    if (!csymbols.length)
                        return;
        ...
        function renderNextCodeCardAsync(cls, options) {
            ...
            var cards = pxt.gallery.parseCodeCardsHtml($el[0]);
            if (!cards) {
        ...
        function readAssetJson(options) {
            ...
            if (options.assetJSONClass) {
                $("." + options.assetJSONClass).each(function (i, c) {
                    var $c = $(c);
                    assetJson = $c.text();
                    c.parentElement.remove();
                });
            ...
            function mergeAssetJson(assetJSON, tilemapJres) {
                if (!assetJSON && !tilemapJres)
                    return undefined;
                var mergedJson = pxt.tutorial.parseAssetJson(assetJSON) || {};
                if (tilemapJres) {
                    var parsedTmapJres = JSON.parse(tilemapJres);
    ...
    (function (runner) {
        ...
        function simulateAsync(container, simOptions) {
            ...
            return __awaiter(this, void 0, void 0, function () {
                var builtSimJS, _c, js, fnArgs, parts, usedBuiltinParts, options, driver, board, storedState, runOptions;
                return __generator(this, function (_d) {
                    switch (_d.label) {
                        case 0:
                            _c = simOptions.builtJsInfo;
                            if (_c) return [3 /*break*/, 2];
                            return [4 /*yield*/, buildSimJsInfo(simOptions)];
                        case 1:
                            _c = (_d.sent());
                            _d.label = 2;
                        case 2:
                            builtSimJS = _c;
                            js = builtSimJS.js, fnArgs = builtSimJS.fnArgs, parts = builtSimJS.parts, usedBuiltinParts = builtSimJS.usedBuiltinParts;
                            if (!js) {
                                console.error("Program failed to compile");
                                return [2 /*return*/, undefined];
                            }
                            options = {};
                            options.onSimulatorCommand = function (msg) {
                            ...
                            driver = new pxsim.SimulatorDriver(container, options);
                            board = pxt.appTarget.simulator.boardDefinition;
                            storedState = getStoredState(simOptions.id);
                            runOptions = {
                            ...
                            driver.run(js, runOptions);
                            return [2 /*return*/, builtSimJS];
                    }
                });
            });
        }
        runner.simulateAsync = simulateAsync;
        function buildSimJsInfo(simOptions) {
            var _a;
            return __awaiter(this, void 0, void 0, function () {
                var didUpgrade, currentTargetVersion, compileResult;
                return __generator(this, function (_b) {
                    switch (_b.label) {
                        case 0: return [4 /*yield*/, loadPackageAsync(simOptions.id, simOptions.code, simOptions.dependencies)];
                        case 1:
                            _b.sent();
                            didUpgrade = false;
                            currentTargetVersion = pxt.appTarget.versions.target;
                            return [4 /*yield*/, compileAsync(false, function (opts) {
                                    var _a;
                                    if (simOptions.assets) {
                                        var parsedAssets = JSON.parse(simOptions.assets);
                                        for (var _i = 0, _b = Object.keys(parsedAssets); _i < _b.length; _i++) {
                                            var key = _b[_i];
                                            var el = parsedAssets[key];
                                            opts.fileSystem[key] = el;
                                            if (opts.sourceFiles.indexOf(key) < 0) {
                                                opts.sourceFiles.push(key);
                                            }
                                            if (/\.jres$/.test(key)) {
                                                var parsedJres = JSON.parse(el);
                                    ...
                                    var sharedTargetVersion = (_a = runner.mainPkg.config.targetVersions) === null || _a === void 0 ? void 0 : _a.target;
                                    if (sharedTargetVersion && currentTargetVersion &&
                                        ...
                                        for (var _c = 0, _d = Object.keys(opts.fileSystem); _c < _d.length; _c++) {
                                            var fileName = _d[_c];
                        ...
                        case 2:
                            compileResult = _b.sent();
                            if (!(((_a = compileResult.diagnostics) === null || _a === void 0 ? void 0 : _a.length) > 0 && didUpgrade)) return [3 /*break*/, 4];
                        ...
                        case 4:
                            ...
                            return [2 /*return*/, pxtc.buildSimJsInfo(compileResult)];
                    }
                });
            });
        }
        ...
        function renderBookAsync(content, summaryid) {
            ...
            return Promise.delay(100)
                ...
                return pxt.U.promisePoolAsync(4, tocsp, function (entry) { return __awaiter(_this, void 0, void 0, function () {
                    var md, e_1;
                    return __generator(this, function (_a) {
                        switch (_a.label) {
                            case 0:
                                _a.trys.push([0, 2, , 3]);
                                return [4 /*yield*/, pxt.Cloud.markdownAsync(entry.path)];
                            case 1:
                                md = _a.sent();
                                entry.markdown = md;
                                return [3 /*break*/, 3];
        ...
        function decompileSnippetAsync(code, options) {
            ...
            return loadPackageAsync(packageid, code)
                ...
                opts.ast = true;
                if (options.assets) {
                    for (var _i = 0, _a = Object.keys(options.assets); _i < _a.length; _i++) {
                        var key = _a[_i];
                        if (opts.sourceFiles.indexOf(key) < 0) {
                            opts.sourceFiles.push(key);
                        }
                ...
                return ts.pxtc.localizeApisAsync(apis, runner.mainPkg)
                    ...
                    var tilemapJres = (_a = options.assets) === null || _a === void 0 ? void 0 : _a[pxt.TILEMAP_JRES];
                    var assetsJres = (_b = options.assets) === null || _b === void 0 ? void 0 : _b[pxt.IMAGES_JRES];
