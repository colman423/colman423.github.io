            ...
            */
            ...
            if (pxt.semver.strcmp(pkgTargetVersion || "0.0.0", "0.11.20") < 0) {
                ...
                pxt.U.toArray(dom.querySelectorAll("variable[type=SpriteKind]")).forEach(function (block) {
                    ...
                    var kindValue = (block.textContent || "").trim();
                    var withoutNum = /[0-9]*([^0-9].*)/.exec(kindValue);
                    if (withoutNum) {
                        legacyKindConversions_1[withoutNum[1]] = kindValue;
                    }
                });
                pxt.U.toArray(dom.querySelectorAll("shadow[type=spritetype], block[type=spritetype]")).forEach(function (block) {
                    var memberField = getField(block, "MEMBER");
                    var cont = ((memberField === null || memberField === void 0 ? void 0 : memberField.textContent) || "").trim();
                    if (legacyKindConversions_1[cont]) {
                        memberField.textContent = legacyKindConversions_1[cont];
                    }
                });
            }
            if (pxt.semver.strcmp(pkgTargetVersion || "0.0.0", "0.18.9") < 0) {
                 ...
                 */
                pxt.U.toArray(dom.querySelectorAll("block[type=spriteshitwall]")).forEach(function (block) {
                    var doc = block.ownerDocument;
                    var tileHitParam = doc.createElement("value");
                    tileHitParam.setAttribute("name", "HANDLER_DRAG_PARAM_location");
                    var shadow = doc.createElement("shadow");
                    shadow.setAttribute("type", "argument_reporter_custom");
                    var mut = doc.createElement("mutation");
            ...
            if (pxt.semver.strcmp(pkgTargetVersion || "0.0.0", "0.18.9") < 0) {
                 ...
                 */
                pxt.U.toArray(dom.querySelectorAll("block[type=tilemap_locationXY]")).forEach(function (block) {
                    var xyField = getField(block, "xy");
